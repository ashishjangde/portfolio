...
export const Velaris = ({
  ...
  useEffect(() => {
    ...
    const backgroundColor = hexToRgb(bg);
    const shaderColors = new Float32Array(colors.slice(0, 4).flatMap(hexToRgb));

    const drawFrame = (time: number) => {
      gl.uniform2f(locs.res, canvas.width, canvas.height);
      gl.uniform1f(locs.time, time * 0.001 * speed);
      gl.uniform1f(locs.grain, grain);
      gl.uniform3f(locs.bg, ...backgroundColor);
      gl.uniform3fv(locs.colors, shaderColors);
    ...
    const start = () => {
      ...
      if (isVisible && !document.hidden && raf === null) {
        raf = window.requestAnimationFrame(render);
      }
    };
  ...
  }, [bg, colors, speed, grain, reduceMotion]);
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        className="pointer-events-none absolute inset-0 h-full w-full opacity-60 dark:opacity-80 transition-opacity duration-500"
      />
